...
public class WordSearch2 {
    ...
    Map<String, List<Pair<int[], Set<Pair<Integer, Integer>>>>> saved = new HashMap<>();
    ...
    public List<String> findWords(char[][] board, String[] words) {
        ...
        for (String word : words) {
            String lp = trie.getLongestPrefix(word);
            if (exist(board, word, lp)) {
                res.add(word);
            } else {
                trie.add(word);
            }
    ...
    public boolean exist(char[][] board, String word, String lp) {
        ...
        if (lp != null && saved.containsKey(lp)) {

            Pair<Integer, Boolean> bfsPair = bfs(board, word, lp.length(), new LinkedList<>(saved.get(lp)));

            if (bfsPair.getValue()) {
                return true;
            } else {
                max = Math.max(bfsPair.getKey(), max);
            }
            ...
            for (int i = 0; i < board.length; i++) {
                for (int j = 0; j < board[0].length; j++) {
                    ...
                    if (board[i][j] == word.charAt(0)) {
                        ...
                        Pair<Integer, Boolean> bfsPair = bfs(board, word, max, list);
        ...
        trie.add(word.substring(0, max + 1));
        return false;

    }

    public Pair<Integer, Boolean> bfs(char[][] board,
                                      String word,
                                      int max,
                                      LinkedList<Pair<int[], Set<Pair<Integer, Integer>>>> list ) {
        ...
        while (!list.isEmpty()) {
            ...
            for (int d = 0; d < directions.length - 1; d++) {
                ...
                if (di >= 0 && di < board.length && dj >= 0 && dj < board[0].length
                    ...
                    saved.putIfAbsent(word.substring(0, cur[2]), new ArrayList<>());

                    if (cur[2] == word.length() - 1) {
                        return new Pair<>(max, true);
                    }

                    Set<Pair<Integer, Integer>> nextVisited = new HashSet<>(pair.getValue());

                    nextVisited.add(new Pair(di, dj));

                    if (max <= cur[2] + 1) {
                        String sub = word.substring(0, cur[2] + 1);
                        saved.putIfAbsent(sub, new ArrayList<>());
                        saved.get(sub).add(new Pair(new int[]{di, dj, cur[2] + 1}, nextVisited));
                        max = cur[2] + 1;
                    }
    ...
    static class Trie {
        ...
        public String getLongestPrefix(String word) {
            ...
            for (int i = 0; i < word.length(); i++) {
                char ch = word.charAt(i);
                if (cur == null || !cur.next.containsKey(ch)) {
                    return res;
                }
                if (cur.next.get(ch).isWord) {
                    res = sb.toString();
                }
                sb.append(ch);
                cur = cur.next.get(ch);
